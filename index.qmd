...
\end{array}\right)
$$ {#eq-Qx} where $\mathbf{V}_x$ is the rate matrix for the transitory (i.e.,
...
A key difference between Approach 3 and Approaches 1 and 2 is that the rows
in $\mathbf{Q}_x$, $\mathbf{V}_x$, and $\mathbf{S}_x$correspond to the final state, while the
columns correspond to the starting state; this is the opposite of
the rate matrices defined for Approaches 1 and 2, where the rows corresponded to the
starting health state and the columns to the ending health state in
a given cycle.

For the defined time step $\Delta_t$, the discrete time
transition probability matrix $\mathbf{P}_x$ is again obtained by taking the
