...
- Just use matrices indexed by $t$ and note that a homogeneous model will have them all the same. 
- Going to need to redo some of the LaTeX figures to do this. 
...
We note that in our implementation of the Sick-Sicker model, we define the disability weight as one minus the utility weight. In general, this will not be the case as disability weights and utility weights are estimated using different methods (based on expert opinion and population surveys, respectively). TK. 
...
params_ <- list(
    ...
    r_HD = .02,  # TK .002 # background mortality rate
...
Each approach facilitates the design and execution of a decision-analytic model that correctly calculates YLD, YLL, and DALY outcomes---as well as other common outcomes such as life-years (LYs), QALYs and costs. In practice, Approaches (1) and (2)  will produce identical results.  Approach (3) draws on slightly different assumptions on partial payoffs for partial occupancy in a cycle, but will yield results very similar to (1) and (2). We show in the results section (@sec-results) that other shortcut-based approaches previously used in the literature---such as modeling a QALY-like DALY and/or accumulating time in the absorbing death state---will not in general yield similar results.  
...
For a time-inhomogeneous model in which transition rates vary as a function of time/age, we can define $\mathbf{Q}_t$ as the transition rate matrix at time $t$. We obtain the transition probability matrix at time $t$ by taking the matrix exponential, i.e., 

$$
\mathbf{P}_t = e^{\mathbf{Q_t}\Delta t}
$$ {#eq-embed-inhomo}


Embedding results in a transition probability matrix $\mathbf{P}$ (or $\mathbf{P}_t$) with the following probabilities defined:
...
Our final approach draws on matrix calculus and adapts methods from mathematical demography to estimate DALY outcomes. While our focus here is on expected outcomes, these methods can also be used to estimate higher order momemts (e.g., variance, skewness); see @caswell2018 and @caswell2021a for details. 

To implement this method we define some additional parameters:
...
\end{aligned}
$$
In the above notation, the matrix $\mathbf{U}_t$ captures transition
probabilities among transient (i.e., non-absorbing) health states, while
$\mathbf{M}_t$ captures transitions from transient health states to the
absorbing death states (non-disease mortality and disease-related mortality). Indexing
by time $t$ indicates that separate matrices are defined for each time step (age class) in the Markov model. 

To construct $\mathbf{U}_t$ and $\mathbf{M}_t$ we define transition rate
("intensity") matrices as in Approaches 1 and 2 above. The overall
...
One important (if minor) difference between the current appraoch and Approaches 1 and 2 is that the rows
in $\mathbf{Q}_t$, $\mathbf{V}_t$, and $\mathbf{S}_t$ correspond to the final state, while the
columns correspond to the starting state; this is essentially the transpose of the rate matrices defined above, where rows corresponded to the initial health state and columns to the final health state. 
...
We can then obtain $\mathbf{U}_t$ and $\mathbf{M}_t$ based on:
...
Markov chain. Borrowing from an example from @caswell2021a, if $\omega=3$ then
...
The block matrix $\tilde{\mathbf{P}}$ is the analogue to the transition matrix $\mathbf{P}$ (@eq-embed) under Approaches 1 and 2 above.
...
With transition matrices and other relevant model objects defined, we next turn our attention to estimating outcomes. Our approaches differ, however, in how total (or expected) outcomes are calculated. Approach 1 requires a Markov trace that tracks occupancy in each cycle; for YLL outcomes, we use this information to calclute the number of new disease-related deaths in each cycle. Approach 2 does not require this extra step, as both cycle-specific and total outcomes are calculated directly. Finally, Approach 3 differs insofar as it solves for expected outcomes (i.e., the approach does not require cycle-specific values as found in a Markov trace).  

### Calculating Cycle Occupancy 

Define $\mathbf{s}_0$ as the initial state occupancy (column) vector at time $t=0$. The vector $\mathbf{s}_0$ has size $k \times 1$, where $k=\tau + \alpha$ is the total number of health states. 
This vector summarizes the number or fraction of the population in each health state at baseline. Often, this vector will be set such that the entire cohort starts off healthy---though this need not always be the case.
...
$$ {#eq-trace} where $\mathbf{P}$ is the $k \times k$ transition probability matrix (@eq-embed). 
...
We can draw on @eq-trace (or @eq-trace-inhomo) to construct an $\omega \times k$ Markov trace $\mathbf{Tr}$. 
...
Total discounted YLDs for a time inhomogenous model are given by,

$$
YLD=\sum_{t=0}^{\omega-1} YLD(t)=\sum_{t=0}^{\omega-1}\left(\mathbf{s}'_0 \mathbf{P}^t \mathbf{d}  \times{e^{-r_{\Delta_t} t}}\right)
$$ {#eq-yldcum}

and 

$$
YLD=\sum_{t=0}^{\omega-1} YLD(t)=\sum_{t=0}^{\omega-1}\left(\mathbf{s}'_0 \mathbf{P}(1)\mathbf{P}(2)\dots\mathbf{P}(t) \mathbf{d}  \times{e^{-r_{\Delta_t} t}}\right)
$$ {#eq-yldcum-inhomo} for a time inhomogeneous model. 

### Calculating YLDs Based on the Markov Trace

An alternative approach to the above is to calcualte YLD outcomes using the Markov trace matrix $\mathbf{Tr}$. 

$$
\mathbf{YLD} = \mathbf{T}\mathbf{d}_{YLD} \times \mathbf{r}
$$ {#eq-yldtrace} where $\mathbf{r}$ is an $\omega \times 1$ discounting vector based on applying the continuous-time discounting formula to each cycle. 

Total discounted YLDs are then calculated as the sum of the resulting vector $\mathbf{YLD}$.

### Years of Life Lost to Disease (YLLs)

As noted in @sec-background and in @eq-yll, YLLs are based on the present value of remaining life expectancy among deaths that occur in each cycle $t$. Define $a(t)$ as the age of the cohort at cycle $t$, i.e., $a(t) = t \cdot \Delta t + a0$, where $a_0$ is the age of the cohort at $t=0$. 

We next define $e(t)=e(a(t))$ as the present value of remaining life expectancy at cycle $t$. Following the GBD continuous time discounting approach, $e(a(t))$ is given by
...
<!-- $$ -->
<!-- \begin{aligned} -->
<!-- E(\tilde{\boldsymbol{\rho}}) & =\tilde{\boldsymbol{\rho}}_1 \\ -->
<!-- V(\tilde{\boldsymbol{\rho}}) & =\tilde{\boldsymbol{\rho}}_2-\left(\tilde{\boldsymbol{\rho}}_1 \circ \tilde{\boldsymbol{\rho}}_1\right) \\ -->
<!-- S D(\tilde{\boldsymbol{\rho}}) & =\sqrt{V(\tilde{\boldsymbol{\rho}})} \\ -->
<!-- C V(\tilde{\boldsymbol{\rho}}) & =\mathcal{D}\left(\tilde{\boldsymbol{\rho}}_1\right)^{-1} S D(\tilde{\boldsymbol{\rho}}) \\ -->
<!-- S k(\tilde{\boldsymbol{\rho}}) & =\mathcal{D}[V(\tilde{\boldsymbol{\rho}})]^{-3 / 2}\left(\tilde{\boldsymbol{\rho}}_3-3 \tilde{\boldsymbol{\rho}}_1 \circ \tilde{\boldsymbol{\rho}}_2+2 \tilde{\boldsymbol{\rho}}_1 \circ \tilde{\boldsymbol{\rho}}_1 \circ \tilde{\boldsymbol{\rho}}_1\right) -->
<!-- \end{aligned} -->
<!-- $$ -->
