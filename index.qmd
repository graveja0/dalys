...
3. Define a block matrix Markov chain with rewards by adapting the methods in @caswell2021a and @caswell2018. This approach draws on matrix calculus and solves for expected outcomes as well as higher order moments such as variance and skewness. 
...
In practice, approaches (1) and (2) above draw on similar transition matrices and payoff vectors, as we will show immediately below. Approach (3) is quite distinct, however, so we will demonstrate this method in Section TK.
...
The matrix $\mathbf{Q}$ has diagonal elements defined as the negative sum of the off-diagonal row values (i.e., the row sums of $\mathbf{Q}$ are zero). This ensures that the Markov model is "closed"---that is, the total cohort size neither grows nor shrinks over time.
...
![Transition Probability Matrix for Approach 1](images/P_model2.png)
...
This section will outline an approach similar to Approach 1, but that draws on a non-Markovian tracking state to track the number of disease-related deaths in each cycle; these counts are then used to match the age of the cohort at each cycle with a reference life table to calculate YLL outcomes. 
...
A state transition diagram with the tracking state added for disease-related deaths is shown in @fig-transition. 
...
![Transition Probability Matrix for Approach 2](images/P_model1.png)
...
With $\mathbf{P}$ defined under either Approach 1 or Approach 2, we now have the necessary ingredients to construct a Markov trace. Define $\mathbf{s}_0$ as the initial state occupancy vector at time $t=0$. The vector $\mathbf{s}_0$ has size $k$, where $k$ is the total number of states captured in $\mathbf{P}$ (including transition states, if using Approach 2). This vector summarizes the number or fraction of the population in each health state at baseline. Often, this vector will be set such that the entire cohort starts off healthy---though this need not always be the case.

For a time-homogenous model such as considered here, health state occupancy at cycle $t$ is defined as the total number or fraction of the cohort in each health state at that time. State occupancy at cycle $t$ is calculated as:
