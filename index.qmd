...
YLLs are determined by a loss function, which is typically defined as the number of years lost to premature mortality at age $a$. This value is often taken directly from a life table that provides information on remaining life expectancy ($Ex(a)$) at a given age. 
...
It is important to note that the discounting shown in @eq-yld and @eq-yll yield the present value of YLD and YLL outcomes at a single point in time, when the duration of disease ($L_c$) and time of death from disease ($a$) are known. For a decision model where not all cohort members start off ill, that point in time very likely occurs at some point after the baseline period---and different illness durations and death times will, of course, occur across individuals in a modeled cohort. As such, to be consistent with standard practice with WHO-CHOICE, we must discount YLL and YLD outcomes further---all the way back to the baseline (time $t=0$) period. This additional discounting step will become apparent in  @sec-outcomes below. 
...
```{r, eval = FALSE}
#https://resource-allocation.biomedcentral.com/articles/10.1186/1478-7547-11-18/tables/3
r = 0.03
Delta_t = 1
t = 10

r_ = r * Delta_t
(1/r_)*(1-exp(-r_*t/Delta_t))*Delta_t
adj = (1/r_)*(1-exp(-r_))
disc = exp(-r_*(0:(t/Delta_t-1)))
sum(rep(1,t/Delta_t)*adj*disc)*Delta_t

# YLL
Ex = 20
(1/r)*(1-exp(-r*Ex))*exp(-r*(t-1))
Ex_ <- (1/r)*(1-exp(-r*Ex))
sum(c(rep(0,(t-1)/Delta_t),Ex_)*disc)

# basic intuition is that discrete time formula (1/(1+r)^t) and continuous time formula exp(-rt) are designed to discount a series of discrete "payoffs." but health doesn't accrue with a single lump-sum value in each cycle; it is continuous flow throughout the cycle. by comparison, the GBD (1/r)(1-exp(-rt)) assumes a continuous flow. we can adjust the cycle "payoff" to reflect a continuous flow by applyign the adjustment factor (1/r)*(1-exp(-r)), and thus once adjusted, any sum total that uses exp(-rt) will be identical as a value that uses (1/r)*(1-exp(-rt)).
...
$$ {#eq-yldtrace}   where $\mathbf{c}$ is an $\omega \times 1$ vector of cycle discoutning/correction factors $c_t$ and $\odot$ is the element-wise multiplication (Hadamard product) operator. 
...
value, variance, etc.). We focus here on expected outcomes (i.e., outcomes based on $\mathbf{R}_1$)---though equations are available to estimate higher-order moments and objects such as variance, skewness, the coefficient of variation, etc. 

### Years of Life Lived With Disease (YLD)

To calculate occupancy-based outcomes such as YLDs, we first define a $\tau \times \omega$ reward matrix
...
as $\mathbf{H}$ but includes the (fully discounted) disability weight:

![YLD reward matrix $\mathbf{V}$](images/H-YLD.png){#fig-H-yld fig-align="center" width="60%"} 

where $\texttt{dwS1}* = \texttt{dwS1} \times \Delta_t  \times  \frac{1}{r_{\Delta_t}}(1-e^{-r_{\Delta_t}}) \times e^{-r_{\Delta_t} t}$, etc.
...
$\widetilde{\mathbf{P}}$ (@eq-Ptilde).

Expected YLDs are estimated by,

$$
\begin{aligned}
& \widetilde{\boldsymbol{\rho}}_{1}=\widetilde{\mathbf{N}}^{\top} \mathbf{Z}\left(\widetilde{\mathbf{P}} \odot \widetilde{\mathbf{R}}_{1}\right)^{\top} \mathbf{1}_{s} 
\end{aligned}
...
HYLD = H
HYLD[1,] = rep(0,ncol(H))
YLD3_ <- params3 %>% healthy_longevity_occupancy(H = HYLD, V = V_YLD)
