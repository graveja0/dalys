...
This tutorial fills the methodological gap described above by outlining a foundation for direct incorporation of DALY outcomes in common decision modeling frameworks. Our primary focus is on how transition matrices and state occupancy payoffs in discrete-time Markov cohort models can be structured to calculate YLLs and YLDs---however, our framework extends directly to microsimulation and (in part) to continuous time discrete event simulation (DES) models. As such, our study provides a comprehensive roadmap for incorporating DALY outcomes into common decision modeling frameworks. 

This tutorial is structured around a simple disease progression model developed to demonstrate how to fit time-dependent cohort state transition models in R. However, the techniques and code provided can be easily adapted for any disease process. We also provide supplemental material in which the same methods are applied to a 
